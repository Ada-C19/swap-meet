...
class Vendor:
    ...
    def swap_items(self, other_vendor, my_item, their_item):
        if their_item not in other_vendor.inventory or my_item not in self.inventory:
            return False
        # if my_item in self.inventory: line 26 checks this 
        self.inventory.remove(my_item)
        other_vendor.inventory.append(my_item)
        # if their_item in other_vendor.inventory: line 26 checks this
    ...
    def swap_best_by_category(self, other_vendor, my_priority, their_priority):
        vendors_item = other_vendor.get_best_by_category(my_priority)
        other_vendors_item = self.get_best_by_category(their_priority)
        return self.swap_items(other_vendor, other_vendors_item, vendors_item)